...
void FindLoops(const std::set<Object>& aObjectSet, const sGuard& aGuard)
{
	for(int i = 0; i < MAP_SIZE.first; i++)
	{
		for(int j = 0; j < MAP_SIZE.second; j++)
		{
			Object AuxObject(i,j);
			if(aObjectSet.count(AuxObject)) continue;

			sGuard AuxGuard = aGuard;
			std::set<Object> AuxObjectSet = aObjectSet;
			// std::cout << "AUX OBJECT (" << AuxObject.first << "," << AuxObject.second << ")" << std::endl;
			// std::cout << "AUX GUARD (" << AuxGuard.mPosition.first << "," << AuxGuard.mPosition.second << ", "<< AuxGuard.mDirection <<" )" << std::endl;

			BUCLE_INFINIT = false;
			AuxObjectSet.emplace(AuxObject);
			CountGuardSteps(AuxObjectSet, AuxGuard);
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	FindLoops(ObjectSet, Guard);


	std::cout << "TOTAL BUCLES INFINITS: " << COUNTER_BUCLE_INFINIT << std::endl;
	return 0;
}